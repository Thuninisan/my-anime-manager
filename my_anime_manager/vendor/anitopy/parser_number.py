...
def _chinese_to_int(text: str) -> int | None:
    ...
    Handles patterns like: 十二 (12), 二十 (20), 一百二十三 (123),
    二十六 (26).
    ...
    if not text:
        return None
    for ch in text:
        if ch not in _CHINESE_DIGITS and ch not in _CHINESE_UNITS:
            return None

    result = 0
    current = 0  # pending digit before a unit
...
def match_type_and_episode_pattern(word, token):
    ...
    if number_begin is None or number_begin < 1:
        return False
...
def match_chinese_numbering_pattern(word, token):
    """Match Chinese numbering patterns like \u7B2C\u5341\u4E8C\u96C6, \u7B2C\u4E09\u5B63, \u5341\u516D\u8BDD.

    Uses Chinese numeral conversion for patterns with or without \u7B2C prefix.
    Returns False when unit is \u5B63 (season) to allow further episode matching.
    """
    if word.isdigit():
        return False

    pattern = r'^(?:\u7B2C)?([0-9]+|[\u4E00\u4E8C\u4E09\u56DB\u4E94\u516D\u4E03\u516B\u4E5D\u5341\u767E\u5343\u4E07\u4E24]+)([\u96C6\u5B63\u8BDD])$'
    match = re.match(pattern, word)
    if not match:
        return False
    ...
    if num_str.isdigit():
        ...
        val = _chinese_to_int(num_str)
        if val is None:
            return False
        final_num = str(val)

    if unit == "\u5B63":
        # Set season but return False to allow further episode matching
        Elements.insert(ElementCategory.ANIME_SEASON, final_num)
        token.category = TokenCategory.IDENTIFIER
        return False
    else:
        # \u96C6 or \u8BDD \u2192 episode number
        set_episode_number(final_num, token, validate=False)
        return True
...
def match_episode_patterns(word, token):
    ...
    if numeric_back:
        if match_number_sign_pattern(word, token):
            return True
    # Japanese counter: 1話, 12話, etc.
    # Chinese numbering: 第十二集, 第三季, 十六话, etc.
    if numeric_front or word.startswith('第'):
        if match_japanese_counter_pattern(word, token):
            return True
        if match_chinese_numbering_pattern(word, token):
            return True
